...
import { redis } from "./redis"; // Import the shared Redis instance
// Remove the Redis initialization from here
...
export async function scrapeUrl(url: string) {
  console.log("scraper.ts Redis URL:", process.env.UPSTASH_REDIS_REST_URL);
  console.log(
    "scraper.ts Redis Token exists:",
  ...
  try {
    ...
    logger.info(`Normalized URL for scraping: ${fullUrl}`);
    ...
    combinedContent = cleanText(combinedContent).slice(0, 60000);
    logger.info("Cleaned content:", combinedContent);
    ...
    logger.info(`Scraped content size: ${combinedContent.length} characters`);
